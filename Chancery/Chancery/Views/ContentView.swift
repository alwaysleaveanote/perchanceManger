...
struct ContentView: View {
    ...
    /// Binding to characters array in DataStore
    private var characters: Binding<[CharacterProfile]> {
        Binding(
            get: { dataStore.characters },
            set: { newValue in
                // Handle array changes by comparing and updating
                for character in newValue {
                    if !dataStore.characters.contains(where: { $0.id == character.id }) {
